    ...
    if (currentHTML !== undefined && currentHTML !== null) {
      ...
      if (matchedIds !== null) {
        matchedIds.forEach(matchedId => {
          currentHTML = currentHTML.replace(matchedId, `<a class="support-link ${this.darkBackground ? 'dark-background' : 'zendesk'}" href="http://zendesk.com/${matchedId}" target="_blank" title="Open in new tab">ZENDESK ${matchedId}</a>`)
        });
      }
